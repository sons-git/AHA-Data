...
@router.post("/{conversation_id}/user/{user_id}/save_file")
async def save_file(user_id: str, file: bytes):
    """
    Save a file into GCS for a given user.

    Args:
        user_id (str): The ID of the user.
        file (bytes): The file content to upload.
    ...
    try:
        if not user_id or not file:
            return build_error_response(
                "INVALID_INPUT",
                "User ID and file content are required",
                400
            )
        
        # Upload the file to GCS
        result = await upload_file_to_gcs(user_id, file)
        ...
        return JSONResponse(
            status_code=200,
            content={"message": "File uploaded successfully", "file_url": result}
        )
